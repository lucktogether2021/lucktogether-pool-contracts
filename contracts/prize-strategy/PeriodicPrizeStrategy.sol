...
abstract contract PeriodicPrizeStrategy is Initializable,
  ...
  bytes32 public adminSeed;
  struct RandomFactor {
    uint256 startBlockNumber;
    string adminRandom;
    bytes32 adminSeed;
  }
  mapping (uint256 => RandomFactor) public randomFactorMap;
  ...
  function setAdminSeed(bytes32 seed) external onlyOwner {
    adminSeed = seed;
  }
  ...
  function startAward() external requireCanStartAward {
    ...
    RandomFactor memory _randomFactor;
    _randomFactor.startBlockNumber = block.number;
    _randomFactor.adminSeed = adminSeed;
    randomFactorMap[requestId] = _randomFactor;
  ...
  function completeAwardSetSeconds(string memory adminRandom, uint256 _prizePeriodSeconds) external onlyOwner {
    _completeAwardWithAdminSeedAndEndSeconds(adminRandom, _prizePeriodSeconds);
  }

  function completeAwardWithAdminSeed(string memory adminRandom) external onlyOwner {
    _completeAwardWithAdminSeedAndEndSeconds(adminRandom, prizePeriodSeconds);
  }

  function _completeAwardWithAdminSeedAndEndSeconds(string memory adminRandom, uint256 _prizePeriodSeconds) internal requireCanCompleteAward {
    require(keccak256(abi.encodePacked(adminRandom)) == adminSeed, "admin seed is not match");
    randomFactorMap[rngRequest.id].adminRandom = adminRandom;
    uint256 _startNumber = randomFactorMap[rngRequest.id].startBlockNumber;
    require(block.number - _startNumber > 10, "complete bigger than start for 10");
    uint256 randomNumber = uint256(keccak256(abi.encodePacked(adminRandom, blockhash(_startNumber),blockhash(_startNumber + 10))));
    _completeAward(randomNumber);
    prizePeriodSeconds = _prizePeriodSeconds;
  }

  function completeAward() external onlyOwner requireCanCompleteAward{
    uint256 randomNumber = rng.randomNumber(rngRequest.id);
    require(randomNumber != 0, "random number is not ready");
    _completeAward(randomNumber);
  }

  /// @notice Completes the award process and awards the winners.  The random number must have been requested and is now available.
  function _completeAward(uint256 randomNumber) internal requireCanCompleteAward {
    delete rngRequest;
    delete adminSeed;
